...
void kernel(const char* command) {
    ...
    for (vmiter it(kernel_pagetable); it.va() < MEMSIZE_PHYSICAL; it += PAGESIZE) {
        ...
        // check to see if console
        if (it.va() == CONSOLE_ADDR) {
            it.map(it.va(), PTE_P | PTE_W | PTE_U);
        }
        // check kernel memory except for console
...
// helper method to help free inside of fork() method that takes in a specific pagetable 
void sys_call_kfree_helper(x86_64_pagetable* pt ) {

    // use vmiter to free the pages in the page table 
    for (vmiter it_one(pt, PROC_START_ADDR); it_one.va() < MEMSIZE_VIRTUAL; it_one += PAGESIZE) {
        // check if not console address and if it is user-accessible 
        if (it_one.va() != CONSOLE_ADDR && it_one.user()) {
            kfree((void*) it_one.kptr());
        }

    }
    // use ptiter to free the pages itself 
    for (ptiter it_two(pt); it_two.active(); it_two.next()) {
        kfree((void*) it_two.kptr());
    }

    // free the rest of the pagetable 
    kfree(pt);
    ...
    for (loader.reset(); loader.present(); ++loader) {
        for (uintptr_t a = round_down(loader.va(), PAGESIZE);
           ...
           // mappings 
            ...
            if (loader.writable()) {
                dstit.map(pa, PTE_P | PTE_W | PTE_U);
            }
            
            else {
                dstit.map(pa, PTE_P| PTE_U);
            }
...
pid_t syscall_fork() {
    ...
    if (!page_table_new) {
        ...
        // Failed to allocate a page for the page table.
        return -1;
    }
    memset(page_table_new, 0, PAGESIZE);

    // Copy mappings from parent into new page table
    vmiter new_vmiter(page_table_new, 0);
    ...
    // vmiter to run through parent process
    for(vmiter parent(current->pagetable, PROC_START_ADDR); parent.va() < MEMSIZE_VIRTUAL; parent+= PAGESIZE, new_vmiter+= PAGESIZE) {
        ...
        if(!parent.writable()){
            ...
            // if user-accessible, then increase the refcount 
            if (parent.user()) {
        ...
        // check if user-accessible but not the console address 
            ...
            // memcopy and then try mapping page 
            memcpy( p, (void*) parent.pa(), PAGESIZE);
            int mapping_result = new_vmiter.try_map(p, parent.perm());
            if (mapping_result == -1) {
                sys_call_kfree_helper(page_table_new);
                kfree(p);
                return -1;
    ...
    // fill in the fields of the proc 
    ptable[pid].pagetable = page_table_new;
    ptable[pid].pid = pid;
    ptable[pid].state = P_RUNNABLE;
